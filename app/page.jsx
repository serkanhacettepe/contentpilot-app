...
function calculateSEOScore(content, keyword) {
  if (!content || !keyword) return { total: 0, breakdown: {} };

  const lower = content.toLowerCase();
  const kw = keyword.toLowerCase();
  const words = content.split(/\s+/).length;
  const kwCount =
    (lower.match(new RegExp(kw.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) || []).length;
  const density = words > 0 ? (kwCount / words) * 100 : 0;
  const hasH2 = /##\s/.test(content) || /<h2/i.test(content);
  const hasH3 = /###\s/.test(content) || /<h3/i.test(content);
  const headingCount = (content.match(/^#{2,3}\s/gm) || []).length;
  const paragraphs = content.split(/\n\n+/).filter((p) => p.trim().length > 20);
  const hasIntro = paragraphs.length > 0 && paragraphs[0].length > 80;
  ...
  const readScore = Math.min(
    25,
    (avgSentenceLen < 25 ? 12 : 6) + (hasIntro ? 5 : 0) + (hasConclusion ? 5 : 0) + (paragraphs.length > 4 ? 3 : 0)
  );
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, fontFamily: "'Courier New', monospace", color: "#fff", fontWeight: 700 }}>
          {score}/{max}
        </span>
      </div>
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["editor", "score", "export"].map((tab) => (
            ...
            >
              {tab === "editor" ? "✏️ Editor" : tab === "score" ? "📊 SEO Score" : "📤 Export"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 57px)" }}>
        ...
        <div style={{ flex: 1, padding: 32, overflowY: "auto" }}>
            ...
            <div>
              ...
              >
                {displayContent.split("\n").map((line, i) => {
                  if (line.startsWith("## ")) {
                    return (
                      <h2 key={i} style={{ fontSize: "1.3rem", fontWeight: 700, color: "#e2e8f0", margin: "28px 0 12px", fontFamily: "'Segoe UI', sans-serif" }}>
                        {line.replace(/^##\s/, "")}
                      </h2>
                    );
                  }
                  ...
                  if (line.startsWith("### ")) {
                    return (
                      <h3 key={i} style={{ fontSize: "1.1rem", fontWeight: 600, color: "#94a3b8", margin: "20px 0 8px", fontFamily: "'Segoe UI', sans-serif" }}>
                        {line.replace(/^###\s/, "")}
                      </h3>
                    );
                  }
          ...
          {activeTab === "export" && (
            <div>
              <h2 style={{ fontSize: "1.4rem", fontWeight: 700, marginBottom: 24 }}>📤 Export & Publish</h2>
              {!generatedContent ? (
                <div style={{ textAlign: "center", padding: 60, color: "#64748b" }}>
